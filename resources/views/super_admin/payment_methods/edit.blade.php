            ...
            <div class="card card-custom min-h-500px">
                <div class="card-body">
                    ...
                    <div class="normal-table-list">
                        <div class="bsc-tbl">
                            <form method="POST" action="{{ route('superadmin.payment_methods.update', $paymentMethod->id) }}"
                                  accept-charset="UTF-8" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="form-group">
                                    <label for="payment_type">Payment Type <span style="color: red; font-size: 18px"><sup>*</sup></span></label>
                                    <input class="form-control" id="payment_type" value="{{ $paymentMethod->payment_type }}"
                                           autocomplete="off"
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" id="descriptions"
                                              autocomplete="off" name="descriptions" type="text">{{ $paymentMethod->descriptions ?? '' }}</textarea>
                                    @error('descriptions')
                                    <strong class="text-danger" role="alert">
                                        <span>{{ $message }}</span>
                                    </strong>
                                ...
                                <div class="form-group">
                                    <div class="checkbox-inline">
                                        <label class="checkbox checkbox-outline checkbox-success">
                                            <input
                                                value="1"
                                                {{ $paymentMethod->status == 1 ? 'checked' : '' }} type="checkbox"
...
@push('script')
    ...
    <script>
        $(document).ready(function () {
            filepond_single_image_configure('{{$paymentMethod->logo_url}}');
