            ...
            <div class="card card-custom min-h-500px">
                <div class="card-body">
                    ...
                    <div class="normal-table-list">
                        <div class="bsc-tbl">
                            <form method="POST"
                                  action="{{ route('superadmin.product_category.update',$parentProductCategory->id) }}"
                                  accept-charset="UTF-8" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="form-group">
                                    <label for="name">Name <span style="color: red; font-size: 18px"><sup>*</sup></span></label>
                                    <input class="form-control" id="name" value="{{ $parentProductCategory->name }}"
                                ...
                                <div class="form-group">
                                    ...
                                    <input class="form-control" id="slug" value="{{ $parentProductCategory->slug }}"
                                    ...
                                    <div class="input-group">
                                        <select class="form-control appendOption" id="parent_category_id" name="parent_category_id">
                                            ...
                                            @foreach($parents_categories as $parents_category)
                                                <option value="{{ $parents_category->id }}" {{$parentProductCategory->parent_category_id == $parents_category->id ? 'selected' : ''}}>{{ $parents_category->name }}</option>
                                                @if($parents_category->childes && $parents_category->childes->count() > 0)
                                                    @foreach($parents_category->childes as $sub)
                                                        <option value="{{ $sub->id }}" {{$parentProductCategory->parent_category_id == $sub->id ? 'selected' : ''}}>&nbsp;--{{ $sub->name }}</option>
                                                    @endforeach
                                                @endif
                                            @endforeach
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control date" id="description"
                                              name="description">{{ $parentProductCategory->description }}</textarea>
                                    @error('description')
                                    <strong class="text-danger" role="alert">
                                        <span>{{ $message }}</span>
                                    </strong>
                                ...
                                <div class="d-flex form-group">
                                    @if($parentProductCategory->thumbnail_image)
                                        <div class="image_square pop_img mr-5" data-img="{{ asset($parentProductCategory->thumbnail_image) }}">
                                            <img
                                                src="{{ asset($parentProductCategory->thumbnail_image) }}"
                                                alt="image">
                                        </div>
                                    @endif
                                    <div class="form-group">
                                ...
                                <div class="form-group">
                                    <div class="checkbox-inline">
                                        <label class="checkbox checkbox-outline checkbox-success">
                                            <input
                                                value="1" {{ $parentProductCategory->status == 1 ? 'checked' : '' }} type="checkbox"
