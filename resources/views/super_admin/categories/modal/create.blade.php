<div class="modal fade" id="ParentCategoryCreateModal" role="dialog">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Parent Category Create</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <div class="table-responsive parentCategoryTable">
                    <form method="POST" action="" accept-charset="UTF-8"
                          enctype="multipart/form-data" id="parentCategoryFormSubmit">
                        @csrf
{{--                        <input type="hidden" name="modalCreate" value="modalCreate">--}}
                        ...
                        <div class="form-group">
                            ...
                            <span id="invalid-description" class="invalid-response text-danger"></span>
                        </div>
                        <div class="form-group">
                            <label for="img">Image </label>
                            <input class="form-control" id="img" value="{{ old('img') }}" autocomplete="off" name="img"
                                   type="file">
                            <span id="invalid-img" class="invalid-response text-danger"></span>
                        </div>
                        <div class="form-group">
                            ...
                            <span id="invalid-status" class="invalid-response text-danger"></span>
                        </div>
                        <button type="button" style="background: #00c292; color: #f0f0f0" class="btn waves-effect" id="parentCategorySubmit">Save
                            Data
                        </button>
                    </form>
                </div>
...
@push('script')
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).off('click', '#parentCategorySubmit').on('click', '#parentCategorySubmit', function () {
                var formData = new FormData($("#parentCategoryFormSubmit")[0]);
                $.ajax({
                    url: "{{ route('superadmin.product_category.modal.store') }}",
                    type: 'post',
                    dataType: 'json',
                    data: formData,
                    cache: false,
