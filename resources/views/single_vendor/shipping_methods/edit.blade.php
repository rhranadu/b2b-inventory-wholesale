            ...
            <div class="card card-custom min-h-500px">
                <div class="card-body">
                    ...
                    <div class="normal-table-list">
                        <div class="bsc-tbl">
                            <form method="POST" action="{{ route('admin.shipping_methods.update', $shippingMethod->id) }}"
                                ...
                                <div class="form-group">
                                    ...
                                    <input class="form-control" id="name" value="{{ $shippingMethod->name }}"
                                           autocomplete="off"
                                           name="name" type="text" @if(\Illuminate\Support\Facades\Auth::user()->vendor_id != $shippingMethod->vendor_id) readonly  @endif>
                                    @error('name')
                                    ...
                                    <div class="bootstrap-select ic-cmp-int">
                                        ...
                                        <select name="vendor_id" id="vendor_id" class="selectpicker form-control" data-live-search="true">
                                            ...
                                            @foreach($vendors as $vendor)
                                                <option value="{{ $vendor->id }}" {{$shippingMethod->vendor_id == $vendor->id ? 'selected' : 'disabled'}} >{{ $vendor->name }}</option>
                                            @endforeach
                                        </select><br/>
                                        @error('vendor_id')
                                ...
                                <div class="form-group">
                                    ...
                                    <input class="form-control" id="charge" value="{{ $shippingMethod->charge }}"
                                           autocomplete="off" name="charge" type="text" @if(\Illuminate\Support\Facades\Auth::user()->vendor_id != $shippingMethod->vendor_id) readonly  @endif>
                                    @error('charge')
                                    <strong class="text-danger" role="alert">
                                        <span>{{ $message }}</span>
                                    </strong>
                                ...
                                <div class="form-group">
                                    ...
                                    <input class="form-control" id="delivery_time" value="{{ $shippingMethod->delivery_time }}"
                                           autocomplete="off" name="delivery_time" type="text" @if(\Illuminate\Support\Facades\Auth::user()->vendor_id != $shippingMethod->vendor_id) readonly  @endif>
                                    @error('delivery_time')
                                    <strong class="text-danger" role="alert">
                                        <span>{{ $message }}</span>
                                    </strong>
