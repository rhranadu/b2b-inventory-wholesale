...
<head>
    ...
    @endif
        @if(isset(auth()->user()->vendor->name))
            <title>POS - {!! auth()->user()->vendor->name !!}</title>
        @endif
    @stack('css')
    <style>
        @font-face {
                        ...
                        <div class="card-body p-0 bg-transparent" style="padding:0 !important;">
                            <div class="card h-100 border-0 bg-transparent">
                                <div class="card-header d-flex justify-content-between align-items-center py-5" style="background: #81abe0 !important;">
                                    <div class="w-50" style="margin: 10px;">
                                    ...
                                    <div class="w-25" style="margin: 10px;">
                                        <div class="input-group">
                                            <div class="input-group-prepend flex-fill">
                                                <select class="form-control product-category" id="productCategory" name="param"></select>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="w-25" style="margin: 10px;">
                ...
                <div class="col-xxl-3 col-xl-4 col-lg-5 col-md-5 h-100" style="padding-left:0!important;">
                    <div class="card h-100 bg-white">
                        <div class="card-header p-3">
                            <div class="input-group">
                                <div class="input-group-prepend flex-fill">
                                    <select class="form-control pos-customer" id="posCustomer" name="param"></select>
                                </div>
                                <div class="input-group-append">
                                    <button class="btn btn-light-primary text-nowrap" id="newCustomer"><i class="fa fa-plus"></i> Customer
                                    </button>
                                </div>
                            </div>
                        </div>
...
<script type="text/javascript" src="{{ asset('assets/plugins/dataTables/js/buttons.colVis.min.js') }}"></script>
{{-- datatable --}}
<script>
    ...
    function loadRandomProducts(searchKey = '', withoutBarcode = '', category = 0, brand = 0) {
        $.ajax({
            url: 'pos/products',
            method: 'get',
            data: {search_key:searchKey, without_barcode:withoutBarcode, product_category_id:category, product_brand_id:brand,},
    ...
    $(document).off('click', '#addRequestModalButton').on('click', '#addRequestModalButton', function (event) {
        $("#returnRequestModal .modal-body").load('{!! url('/pos/return-request') !!}',function(){
    ...
    $(document).off('click', '#backFromRequestDetail, #backFromEditRequest').on('click', '#backFromRequestDetail, #backFromEditRequest', function (event) {
        $("#pending_return_request_modal .modal-body").load('{!! url('/pos/pending-request') !!}');
    });
    $(document).off('click', '.edit-return-request').on('click', '.edit-return-request', function (event) {
        $("#pending_return_request_modal .modal-body").load('{!! url('/pos/edit-return-request/') !!}'+$(this).data('key'),function(){
            checkReturnRequestValidation();
    ...
    {{--    $.ajax({--}}
    {{--        url: "{!! url('/pos/exchange-product-detail')  !!}",--}}
    {{--        type: 'get',--}}
    ...
    {{--    $.ajax({--}}
    {{--        url: "{{ url('pos/return-request-detail') }}",--}}
    {{--        type: 'get',--}}
    ...
    {{--    $.ajax({--}}
    {{--        url: "{{ url('pos/return-request-submit') }}",--}}
    {{--        type: 'post',--}}
    {{--        data: formData,--}}
    {{--        dataType: 'json',--}}
    {{--        processData: false,--}}
    ...
    $(document).off('click', '.delete-return-request').on('click', '.delete-return-request', function () {
        var url = "{{url('pos/destroy-return-request')}}/"+$(this).data('key');
    ...
    $(".pos-customer").select2({
        width: '100%',
        ajax: {
            url: "{{ route("admin.poscustomer.list") }}",
            dataType: 'json',
            type: 'GET',
    ...
    $(".product-category").select2({
        ...
        allowClear: true,
        ajax: {
            url: "{{ route("admin.pos-product-category.list") }}",
            dataType: 'json',
            type: 'GET',
            data: function (params) {
                return {
    ...
    $(".product-brand").select2({
        ...
        allowClear: true,
        ajax: {
            url: "{{ route("admin.pos-product-brand.list") }}",
            dataType: 'json',
            type: 'GET',
            data: function (params) {
                return {
    ...
    function everySingleProduct(element, id) {
        ...
        if (available_quantity > 0) {
            ...
            if (findTr.length > 0) {
                ...
                var category_id = $('#productCategory :selected').val();
    ...
    $(document).on('click', '.PayButton:visible', function (e) {
        ...
        var pos_customer_id = $("#posCustomer").val();
        var pos_customer_name = $("#posCustomer :selected").text();
...
<script>
        ...
        $.post("{{ route('admin.sale.payment.store.with_ajax') }}",
            ...
            function (res) {
                if (res == 'payment_success')
                {
                    ...
                    $("#sale_invoice_receipt .modal-body").load('{!! url('/pos/receipt') !!}' + '/' + last_sale_id,function(){
    ...
    function openPaymentModal(saleId) {
        $(".payment_model .modal-body").load('{!! url('/pos/payment_option') !!}' + '/' + saleId,function(){
